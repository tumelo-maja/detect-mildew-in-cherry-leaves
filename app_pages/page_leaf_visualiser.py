...
def page_leaf_visualiser_body(version="v1"):
    ...
    st.info(
       "The main goal is to visually differentiate leaves affected"
    ...
    if st.checkbox("Average and variability images"):
        ...
        avg_healthy = plt.imread(
            f"outputs/{version}/average_variability_healthy.png")
        st.image(
            avg_infected, caption="Infected Leaves - Average and Variability")
        st.image(
            avg_healthy, caption="Healthy Leaves - Average and Variability")
        st.write("---")

    if st.checkbox("Difference between average infected and healthy leaves"):
    ...
    if st.checkbox("Create Image Montage"):
        ...
        title = "Infected Leaves" if 'mildew' in label else "Healthy Leaves"

        selected_size = st.selectbox(
            "Choose montage layout", ["2 x 2", "2 x 3", "3 x 3"], index=1)
...
def image_montage(dir_path, label, nrows, ncols, fig_width=10, title_str=''):
    sns.set_style("white")
    ...
    if label in labels:
        ...
        images_list = os.listdir(dir_path + '/' + label)
        ...
        img = imread(dir_path + '/' + label + '/' + img_idx[0])
        ...
        for x in range(0, nrows*ncols):
            img = imread(dir_path + '/' + label + '/' + img_idx[x])

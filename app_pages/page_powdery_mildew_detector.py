...
def page_powdery_mildew_detector_body(version="v1"):
    ...
    st.info(
        f"Detection tool allows one more images of leaves to be "
        "uploaded to detect if the leaf in the image has "
        "powdery mildew or not.\n\n"
        "Upload one or more leaf images to check "
        "whether they are healthy or infected.")
    ...
    if uploaded_images is not None:
        ...
        for image in uploaded_images:
            ...
            size_str = "{img_width}px width x {img_height}px height"
            st.image(
                img_pil,
                caption=f"Image Size: {size_str}"
            )

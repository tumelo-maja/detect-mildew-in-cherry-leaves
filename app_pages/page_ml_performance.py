...
def page_ml_performance_diagnostics(version="v1"):
    ...
    st.image(
        dist_img,
        caption="Images per dataset split (Train / Validation / Test)")
    ...
    st.write(
        ...
        "the validation set helps tune it, "
    ...
    st.write(
        ...
        "The model shows strong ability to "
        "distinguish healthy leaves from infected leaves."
    )
    st.write("---")
    ...
    st.write(
        "The ROC curve indicates how well "
        "the model separates the two classes. "
        "The confusion matrix shows where "
        "the model makes correct and incorrect predictions."
    )
    st.write("---")

    st.write("### Training Progress Over Time")
    perf_img = plt.imread(
        f"outputs/{version}/model_training_loss_accuracy.png")
    st.image(perf_img, caption="Training vs Validation Loss and Accuracy")

    st.write(
        "A smooth training curve with similar "
        "training and validation performance "
        "suggests the model has learned general patterns "
        "rather than memorizing the dataset."
    )
    st.write("---")
    ...
    st.write(
        "These results show how the model performs "
        "on new images that were not part of training."
    )
